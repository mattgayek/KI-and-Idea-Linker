...
#need dictionary of all TSI client logins then iterate through for loop setting username.send_keys and password.send_keys to dictionary values and keys put all others inside loop add descriptive errors if failure
def ki_linker(input_ki):
	...
	for row in user_rows:
		...
		for key, value, in row.items():
			...
			#Will become a for loop to loop over every KI and make the variable the current KI
				...
				driver.find_element_by_link_text(ki).click()
				driver.implicitly_wait(2)
				try:
					driver.implicitly_wait(3)
					driver.find_element_by_name("this_known_issue_affects_me").click()
					driver.implicitly_wait(2)
